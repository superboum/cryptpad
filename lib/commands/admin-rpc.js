...
var getActiveSessions = function (Env, Server, cb) { // XXX
    var stats = Server.getSessionStats(); // XXX
    cb(void 0, [
        stats.total,
        stats.unique
    ]);
...
var getRegisteredUsers = function (Env, Server, cb) { // XXX expose as API somehow? hk-util ?
    Env.batchRegisteredUsers('', cb, function (done) {
        ...
        nThen(function (waitFor) {
            ...
            folders.forEach(function (f) {
                var dir = Env.paths.pin + '/' + f; // XXX path.join
                Fs.readdir(dir, waitFor(function (err, list) {
                    if (err) { return; }
                    users += list.length;
                }));
...
var getActiveChannelCount = function (Env, Server, cb) {
    cb(void 0, Server.getActiveChannelCount()); // XXX include in stats
};
...
var getUserTotalSize = function (Env, Server, cb, data) { // XXX add logging
    ...
    Pinning.getTotalSize(Env, signingKey, cb); // XXX only works for some users????
};

var commands = {
    ACTIVE_SESSIONS: getActiveSessions, // XXX
    ACTIVE_PADS: getActiveChannelCount, // XXX
