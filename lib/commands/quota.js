...
var queryAccountServer = function (Env, daily, cb) {
    ...
    var path = '/api/getauthorized';
    if (Env.allowSubscriptions) {
        path = '/api/getquota';
    }


    var options = {
        host: Env.accountsDomain || 'accounts.cryptpad.fr',
        path: path,
        method: 'POST',
        headers: {
...
Quota.queryAccountServer = function (Env, daily, cb) {
    Env.batchAccountQuery('', cb, function (done) {
        queryAccountServer(Env, daily, done);
    });
};
...
Quota.updateCachedLimits = function (Env, daily, _cb) {
    var cb = Util.mkAsync(_cb);

    Quota.applyCustomLimits(Env);

    if (!Quota.shouldContactServer(Env)) { return void cb(); }
    Quota.queryAccountServer(Env, daily, function (err, json) {
...
Quota.getUpdatedLimit = function (Env, safeKey, cb) {
    Quota.updateCachedLimits(Env, false, function (err) {
        if (err) { return void cb(err); }
