...
var queryAccountServer = function (Env, quota, cb) {
...
Quota.queryAccountServer = function (Env, quota, cb) {
    Env.batchAccountQuery('', cb, function (done) {
        queryAccountServer(Env, quota, done);
    });
};
...
Quota.dailyPing = function (Env, cb) {
    if (!Quota.shouldContactServer(Env)) { return void cb(); }
    Quota.queryAccountServer(Env, false, function (err) {
...
Quota.updateCachedLimits = function (Env, _cb) {
    ...
    if (!Quota.shouldContactServer(Env)) { return void cb(); }
    Quota.queryAccountServer(Env, true, function (err, json) {
        if (err) { return void cb(err); }
